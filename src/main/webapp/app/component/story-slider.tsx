  ...
  return (
    <Slider {...storysettings}>
      ...
      {storyList.map((value, index) => (
        <div key={index}>
          <div className='card w125 h200 d-block border-0 shadow-xss rounded-xxxl bg-gradiant-bottom overflow-hidden cursor-pointer mb-3 mt-0 me-3'
               style={{backgroundImage: `url("assets/images/${value.bgUrl}")`}}>
            <div className='card-body d-block p-3 w-100 position-absolute bottom-0 text-center'>
              <figure className='overflow-hidden avatar ms-auto me-auto mb-0 position-relative w50 z-index-1'><img
                src={`assets/images/${value.imageUrl}`} alt='avater' className='float-right p-0 bg-white rounded-circle w-100 shadow-xss' /></figure>
              <div className='clearfix mt-1'></div>
