...
export const PostComment = (props: RouteComponentProps<{ url: string }>) => {
  ...
  const links = useAppSelector(state => state.postComment.links);
  const entity = useAppSelector(state => state.postComment.entity);
  ...
  return (
    <div>
      <h2 id="post-comment-heading" data-cy="PostCommentHeading">
        <Translate contentKey="catinyApp.postComment.home.title">Post Comments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="catinyApp.postComment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="catinyApp.postComment.home.createLabel">Create new Post Comment</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('catinyApp.postComment.home.search')}
                />
          ...
          {postCommentList && postCommentList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="catinyApp.postComment.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('uuid')}>
                    <Translate contentKey="catinyApp.postComment.uuid">Uuid</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
              ...
              <tbody>
                {postCommentList.map((postComment, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    <td>
                      <Button tag={Link} to={`${match.url}/${postComment.id}`} color="link" size="sm">
                        {postComment.id}
                      </Button>
                    </td>
                    ...
                    <td className="text-right">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button tag={Link} to={`${match.url}/${postComment.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                        ...
                        <Button
                          tag={Link}
                          to={`${match.url}/${postComment.id}/delete`}
                          color="danger"
                          size="sm"
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="catinyApp.postComment.home.notFound">No Post Comments found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
