...
class CatinyKafkaResourceIT {
  ...
  void setup() {
    ...
    CatinyKafkaResource kafkaResource = new CatinyKafkaResource(kafkaProperties);
  ...
  void producesMessages() throws Exception {
    restMockMvc
      .perform(post("/api/catiny-kafka/publish/topic-produce?message=value-produce"))
  ...
  void consumesMessages() throws Exception {
    ...
    MvcResult mvcResult = restMockMvc
      .perform(get("/api/catiny-kafka/consume?topic=topic-consume"))
      .andExpect(status().isOk())
