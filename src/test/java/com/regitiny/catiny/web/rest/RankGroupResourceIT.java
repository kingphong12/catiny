...
class RankGroupResourceIT {
  ...
  void getAllRankGroupsByRankUserIsEqualToSomething() throws Exception {
    ...
    if (TestUtil.findAll(em, RankUser.class).isEmpty()) {
      rankUser = RankUserResourceIT.createEntity(em);
      em.persist(rankUser);
      em.flush();
    } else {
      rankUser = TestUtil.findAll(em, RankUser.class).get(0);
    }
    em.persist(rankUser);
    em.flush();
  ...
  void searchRankGroup() throws Exception {
    ...
    // Initialize the database
    rankGroupRepository.saveAndFlush(rankGroup);
    when(mockRankGroupSearchRepository.search("id:" + rankGroup.getId(), PageRequest.of(0, 20)))
