...
describe('TodoList e2e test', () => {
  ...
  it('should create an instance of TodoList', () => {
    ...
    cy.get(`[data-cy="uuid"]`)
      .type('82b8d82e-26e8-4d3c-986a-70b5c51c8418')
      .invoke('val')
      .should('match', new RegExp('82b8d82e-26e8-4d3c-986a-70b5c51c8418'));

    cy.get(`[data-cy="title"]`).type('transmitting Pizza Chips').should('have.value', 'transmitting Pizza Chips');
  ...
  it('should delete last instance of TodoList', function () {
    cy.intercept('GET', '/api/todo-lists/*').as('dialogDeleteRequest');
    cy.visit(todoListPageUrl);
    cy.wait('@entitiesRequest').then(({ response }) => {
      if (response.body.length > 0) {
